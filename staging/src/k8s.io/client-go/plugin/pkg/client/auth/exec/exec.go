...
func (c *cache) put(s string, a *Authenticator) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.m[s] = a
}

// NewAuthenticator returns an exec-based client auth provider.
func NewAuthenticator(config *api.ExecConfig) (*Authenticator, error) {
...
func newAuthenticator(c *cache, config *api.ExecConfig) (*Authenticator, error) {
	...
	if config.Command == "" {
		return nil, fmt.Errorf("exec plugin: no command to execute")
	}

	if config.APIVersion == "" {
		return nil, fmt.Errorf("exec plugin: no apiVersion specified")
	}
	...
	for _, env := range config.Env {
		if env.Name == "" {
			return nil, fmt.Errorf("exec plugin: got environment variable with no name")
		}
		if env.Value == "" {
			return nil, fmt.Errorf("exec plugin: environment variable %q has no value", env.Name)
		}
...
// Authenticator is a client auth provider that rotates credentials by executing a plugin.
...
func (r *roundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if res.StatusCode == http.StatusUnauthorized {
		...
		if err := r.a.refresh(resp); err != nil {
...
func (a *Authenticator) refresh(r *clientauthentication.Response) error {
	a.mu.Lock()
	defer a.mu.Unlock()
