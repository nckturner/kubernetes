...
// Enabled implements DockerConfigProvider.Enabled. Enabled is true if we detect
...
func tryValidateEC2UUID() bool {
	...
	if _, err := os.Stat(asset_tag); err == nil {
		...
		if err != nil {
			klog.Errorf("error checking if this is an EC2 instance: %v", err)
		} else if strings.HasPrefix(string(b), "i-") && (len(string(b)) == 19 || len(string(b)) == 10) {
			// Instance ID's are either 19 characters (new format) or 10 characters (old format)
